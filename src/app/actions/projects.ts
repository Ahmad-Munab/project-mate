...
export async function getProjects() {
    try {
        ...
        try {
            console.log(
                "Server: Attempting to fetch projects for user:",
                user.id
            );
            ...
            const mappedProjects = userProjects.map(
                ({ project, memberRole, ownerId }) => ({
                    id: project.id,
                    ...
                    ownerInfo:
                        ownerId !== user.id
                            ? {
                                  name:
                                      user.email?.split("@")[0] ||
                                      "Unknown User", // Or any other user display name logic
